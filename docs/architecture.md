...
| `stylemate/rightapi_images.py` | RightAPI 异步图片任务提交、续查与安全下载 |
| `stylegen/` | 独立的训练数据读取和 StyleGen 数据契约 |

## Provider 抽象

`APIConfig` 将文本和图片的 Key、Base URL、模型及文本协议归一化。默认路径使用 OpenAI Python SDK，因此未列名的服务只要实现所需协议，就作为通用 OpenAI-compatible endpoint 使用。只有协议确实不同的 RightAPI 异步图片流程保留专用适配器。

这种边界避免把私人网关或普通兼容服务写成架构组件。新增普通兼容服务不需要改源码；只有不同的请求、轮询或返回契约才需要单独适配和测试。

## 会话与历史

- API Key、上传图片、识别结果、搭配、收藏和异步任务保存在 Streamlit 会话内存。
- 当前实现没有用户账号或持久化数据库；刷新、断线、会话过期或服务重启可能丢失状态。
- 历史按本地生成的批次 ID 隔离，避免模型重复返回 LOOK 编号时相互覆盖。
- 已开始的异步任务保留当时的 API 配置，后续修改设置不会把新 Key 用于旧任务。
