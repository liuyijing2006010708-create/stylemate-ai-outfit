...
(async () => {
  ...
  try {
    ...
    await page.getByRole('button', { name: '✦ 查看固定 Demo', exact: true }).waitFor({ state: 'hidden' });
    await page.getByRole('button', { name: '收藏', exact: true }).first().waitFor();
    await page.waitForTimeout(3500);
    await page.getByText('版本 0.2.0-rc.1', { exact: true }).scrollIntoViewIfNeeded();
